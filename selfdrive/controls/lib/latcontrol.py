...
def calc_states_after_mpc(states, v_ego, steer_angle, curvature_factor, steer_rate, steer_ratio, delay):
  states[0].x = v_ego * delay
  states[0].psi = v_ego * curvature_factor * math.radians(steer_angle + steer_rate * delay) # / steer_ratio
  return states
...
class LatControl(object):
  ...
  def update(self, active, v_ego, angle_steers, angle_rate, steer_override, d_poly, angle_offset, CP, VM, PL):
    ...
    if self.last_mpc_ts < PL.last_md_ts:
      ...
      #delayFactor = 0.5 + self.ratioScale * abs(angle_steers / 100.) ** self.ratioExp

      # account for actuation delay
      self.cur_state = calc_states_after_delay(self.cur_state, v_ego, (angle_rate * CP.steerActuatorDelay + angle_steers), self.curvature_factor, CP.steerRatio, CP.steerActuatorDelay)
