...
class LongitudinalMpc(object):
  ...
  def get_acceleration(self):  # calculate car's own acceleration to generate more accurate following distances
    ...
    a = a / (len(
      self.velocity_list) / 100.0)  # divide difference in velocity by how long in seconds the velocity list has been tracking velocity (2 sec)
  ...
  def generate_cost(self, distance):
    ...
    return round(float(np.interp(distance, x, y)), 2)
  ...
  def generateTR(self, velocity, relative_velocity):  # in m/s
    ...
    y = [1.03, 1.05363, 1.07879, 1.11493, 1.16969, 1.25071, 1.36325, 1.43, 1.6, 1.7, 1.75618, 1.85, 2.0]  # distance
    ...
    y = [(TR + .425), (TR + .320565), (TR + .257991), (TR + .126369), TR, (TR - .18), (TR - .3)]  # modification values
  ...
  def update(self, CS, lead, v_cruise_setpoint, relative_velocity):
    ...
    TR = self.generateTR(v_ego, relative_velocity)
    generated_cost = self.generate_cost(TR)
